...
# Path to load Stable Diffusion models
cache_dir = ".hf_cache/" 
...
pipe_sdxl_controlnet.enable_model_cpu_offload() # For better memory efficiency

path_img_style = './prompts/' # Path where image styles are located
prompt_presets = load_prompt_presets(path_img_style) # Load image style prompts
preset_list = list(prompt_presets.keys()) # List image styles

# From Input to Image Styles Image
def diffusion_model(img_input, style_group, steps_slider, guidance_scale, controlnet_conditioning_scale):
    ...
    image = cv2.Canny(img_input, 100, 200) # transform input to canny image
    image = image[:, :, None]
